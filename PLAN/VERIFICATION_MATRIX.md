...
| 15 | T-4 fetchUrl/webSearch envelope + provenance escalation | STABILITY_SECURITY.md T-4 | src/agent/web.js (UNTRUSTED tags, trustLevel, provenance); src/agent/governance.js classifySpend escalates when request.provenance === "external_untrusted" — bypasses allowedWithoutApproval fast path | unit | tests/governance-t4-provenance.test.js 10/10; web.js coverage in scam + fetch tests | PASS |
...
| 19 | SDK public (@orbithouse/sdk) | D-009 | packages/orbit-sdk/index.js:1017 module.exports (create, createOrbitClient, exportBundle, projectForDashboard, projectHandoffSlim, projectSpawnSlim, projectFamilySlim, projectErrorsSlim, FILES) | unit | tests/orbit-sdk*.test.js 25/25 | PASS |
...
| 25 | Proof-viewer published to npm | Phase 1 / S-003 | packages/proof-viewer/package.json | static | `npm view @orbithouse/proof-viewer` returns 404; not published | PARTIAL-DEFERRED |
...
| 29 | T-6 AI key rotation advisory | STABILITY_SECURITY.md T-6 | src/agent/ai-key-rotation.js evaluateAiKeyRotation() + ROTATION_ISSUE_LABEL; producer scripts/rotate-ai-key.js + `npm run rotate:ai-key`; cycle-wiring not yet (predicate dormant) | unit | tests/ai-key-rotation.test.js 9/9; producer smoke `--dry-run` OK; consumer wire follow-up | PASS (advisory + producer; wire pending) |
